                ...
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable new_datatable_table">
                            <thead>
                            <tr>
                                <th>@lang('admin.violation_number')</th>
                                <th>@lang('admin.violation_date')</th>
                                <th>@lang('admin.violation_value')</th>
                                <th>@lang('admin.violation_status')</th>
                                <th>@lang('admin.vehicleNumber')</th>
                                <th>@lang('admin.trafficID')</th>
                                <th>@lang('admin.violation_image')</th>
                            ...
                            <tbody>
                                @foreach($trafficViolations as $index=>$row)

                                    <tr>
                                        <td>{{$row->violation_number}}</td>
                                        <td>{{$row->violation_date}}</td>
                                        <td>{{$row->violation_value}}</td>
                                        <td>@lang('admin.'.$row->violation_status)</td>
                                        <td>{{$row->vehicleNumber}}</td>
                                        <td>{{$row->trafficID}}</td>
                                        @if(empty($row->violation_image))
                                        <td> صورة المخالفه غير متوفرة </td>
                                        @else
                                        <td>
                                            <a class="btn btn-info btn-sm" href="{{ url('/dashboard/uploads/violationImage/violation_image/'. $row->violation_image) }}" target="_blank">تحميل الملف</a>
                                        </td>
                                        @endif

                                        <td>
                                                <a href="{{ url('admin-panel/traffic-violations/edit',$row->id) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
                                        </td>
                                    </tr>
