...
@section('content')
    ...
    <div class="clearfix row">
        <div class="col-lg-12">
            <div class="card">
                <div class="header">
                    <h2>@lang('admin.'.$model)<small>@lang('admin.table_desc')</small> </h2>
                    <br>
                        ...
                        <table class="table table-bordered table-striped table-hover dataTable new_datatable_table">
                            ...
                            <tbody>
                                @foreach($rows as $term)
                                    <tr>
                                        <td>{{ \Illuminate\Support\Str::limit($term->name_ar,50) }}</td>
                                        <td>{{ \Illuminate\Support\Str::limit($term->name_en,50) }}</td>

                                        <td>
                                            <form action="{{ route('dashboard.' . $model . '.destroy',$term->id) }}" method="post">
                                                @csrf
                                                @method('delete')
                                                <a href="{{ route('dashboard.' . $model . '.edit',$term->id) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
