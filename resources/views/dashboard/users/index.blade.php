                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable new_datatable_table">
                            ...
                            <tr>
                                ...
                                <th>@lang('admin.personal_photo')</th>
                            ...
                            <tbody>
                                @foreach($rows as $index=>$row)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $row->email != null ? $row->email : 'لا يوجد' }}</td>
                                        <td>{{ $row->phone }}</td>
                                        <td><img src="{{  url('dashboard/uploads/users/'.$row->profile->personal_photo)  }}" style="width: 70px;height: 70px;border-radius: 30px;"></td>
                                        @if($row->type == 1)
                                            <td><button class="btn btn-dark disabled">@lang('admin.delivery_employees')</button></td>
                                        @elseif($row->type == 2)
                                            <td><button class="btn btn-info disabled">@lang('admin.delivery_representatives')</button></td>
                                        @elseif($row->type == 3)
                                            <td><button class="btn btn-warning disabled">@lang('admin.companies')</button></td>
                                        @elseif($row->type == 4)
                                            <td><button class="btn btn-primary disabled">@lang('admin.individuals')</button></td>
                                        @endif
                                        <td>
                                            <form action="{{ route('dashboard.' . $model . '.destroy',$row->id) }}" method="post">
                                                ...
                                                <a href="{{route('dashboard.delegate.orders', $row->id) }}" class="btn btn-primary"><i class="fa fa-first-order"></i></a>
                                                @endif
                                                <a href="{{ route('dashboard.' . $model . '.show',['user'=>$row->id,'country_id'=>$row->profile->country_id,'city_id'=>$row->profile->city_id,'state_id'=>$row->profile->state_id]) }}" class="btn btn-info"><i class="fa fa-eye"></i></a>
                                                <a href="{{ route('dashboard.' . $model . '.edit',['user'=>$row->id,'country_id'=>$row->profile->country_id,'city_id'=>$row->profile->city_id,'state_id'=>$row->profile->state_id]) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
