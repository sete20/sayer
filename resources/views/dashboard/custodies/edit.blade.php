                ...
                <div class="body">
                    ...
                    <form action="{{ route('dashboard.'. $model . '.update',$row->id) }}" method="post">
                            ...
                            <div class="col-md-6 input-group mb-3">
                                ...
                                <select name="company_asset_id" class="form-control">
                                    ...
                                    @foreach($companyAssets as $asset)
                                        <option {{ $row->company_asset_id == $asset->id ? 'selected' : '' }} value="{{ $asset->id }}">{{ $asset->assetName }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 input-group mb-3">
                                ...
                                <select name="user_id" class="form-control">
                                    ...
                                    @foreach($users as $user)
                                        <option {{ $row->user_id == $user->id ? 'selected' : '' }} value="{{ $user->id }}">{{ $user->email }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="input-group mb-3">
                            ...
                            <input type="number" value="{{ $row->quantity }}" name="quantity" class="form-control">
                        </div>
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text">@lang('admin.delivery_date')</span>
                            </div>
                            <input type="date" value="{{ $row->delivery_date }}" name="delivery_date" class="form-control">
                        </div>
