                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable new_datatable_table">
                            ...
                            <tr>
                                ...
                                <th>@lang('admin.document_image')</th>
                                <th>@lang('admin.registrationDate')</th>
...
{{--                                            <img width="200px" height="100px" src="" alt="">--}}
                                            <a class="btn btn-info btn-sm" href="{{ url('dashboard/uploads/companyDocuments/DocumentImage/'. $document->document_image) }}" target="_blank">تحميل الملف</a>
                                            </td>
                                        <td>{{ $document->created_at->diffForHumans() }}</td>
                                        <td>
                                            <form action="{{ route('dashboard.' . $model . '.destroy',$document->id) }}" method="post">
                                                @csrf
                                                @method('delete')
                                                <a href="{{ route('dashboard.' . $model . '.edit',$document->id) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
