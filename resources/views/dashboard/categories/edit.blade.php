                    ...
                    <form action="{{ route('dashboard.'. $model . '.update',$row->id) }}" method="post">
                        ...
                        <ul class="nav nav-tabs">
                            @foreach(config('translatable.locales') as $locale)
                                <li class="nav-item"><a class="nav-link @if($loop->first) active show @endif" data-toggle="tab" href="#{{ $locale }}">@lang('admin.'.$locale)</a></li>
                            @endforeach
                        </ul>
                        <div class="tab-content">
                            @foreach(config('translatable.locales') as $locale)
                                <div class="tab-pane @if($loop->first) show active @endif" id="{{ $locale }}">
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text">@lang('admin.title') ( {{ $locale }} )</span>
                                        </div>
                                        <input type="text" name="{{ $locale }}[title]" class="form-control" value="{{ $row->translate($locale)->title }}" placeholder="@lang('admin.title')" aria-label="Username" aria-describedby="basic-addon1">
                                    </div>
